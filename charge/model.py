...
class LSTM_MODEL(object):
	...
	def __init__(self, placeholders, word_embeddings, attr_table, config):
		...
		lstm_fw_cell = tf.nn.rnn_cell.BasicLSTMCell(size, forget_bias = 0.0)
		lstm_bw_cell = tf.nn.rnn_cell.BasicLSTMCell(size, forget_bias = 0.0)
		...
		cell_fw = tf.nn.rnn_cell.MultiRNNCell([lstm_fw_cell] * config.num_layers)
		cell_bw = tf.nn.rnn_cell.MultiRNNCell([lstm_bw_cell] * config.num_layers)
		...
		outputs,_ = tf.nn.bidirectional_dynamic_rnn(cell_fw,cell_bw,inputs,initial_state_fw = self.initial_state_fw,initial_state_bw = self.initial_state_bw,sequence_length=self.input_length)
		...
		output = tf.expand_dims(tf.reshape(tf.concat(1,values=outputs), [batch_size, -1, size*2]), -1)
		...
		with tf.name_scope("lstm_maxpool"):
			output_pooling = tf.nn.max_pool(output,
					...
					padding='VALID',
					name="pool")
			self.output = tf.reshape(output_pooling, [-1, size*2])

		with tf.name_scope("text-gcn"):
			self.placeholders = placeholders
			self.gcn_inputs = tf.placeholder(tf.float32, shape=(None,size*2))
			self.inputs = tf.concat(0,[self.output,self.gcn_inputs])
			self.input_dim = 200
			self.output_dim = 149
			self.layers = []
			self.activations = []
			self.gcn_outputs = None
			self.output_mid = None
			self.gcn_loss = 0
			self.gcn_accuracy = 0
			...
			self.layers.append(GraphConvolution(input_dim=config.hidden1,
			...
			self.gcn_outputs = self.activations[-1]
			self.output_mid = self.activations[1]
			self.output_mid = tf.slice(self.output_mid,[0,0],[batch_size,size*2])
			self.gcn_loss = masked_softmax_cross_entropy(self.gcn_outputs,self.placeholders)
			self.gcn_accuracy = masked_accuracy(self.gcn_outputs, self.placeholders)

			
		with tf.name_scope("lstm_output"):
			softmax_w = tf.get_variable("softmax_w", [(size+size+size)*2, num_classes])
			...
			temp = [self.output]
			temp.append(tf.reshape(ave_attention_outputs, [batch_size, size*2]))
			temp.append(self.output_mid)
		...
		with tf.name_scope("loss"):
			...
			self.total_loss = self.lstm_loss + config.attr_loss_ratio*self.total_attr_loss + config.gcn_loss_ratio*self.gcn_loss
	...
	def attention(self,inputs,attention_size):
		...
		for temp in range(self.num_attrs):
			with tf.name_scope("attr_attention"+str(temp)):
				...
				#score
				attr_pred_w = tf.get_variable(name='attn_pred_w'+str(temp),shape=[self.size*2,2])
