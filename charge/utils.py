...
def load_corpus(dataset_str):
    ...
    objects = []
    for i in range(len(names)):
        with open("/mnt/d/jjpeng/ljzhao/text-gcn/data/ind.{}.{}".format(dataset_str, names[i]), 'rb') as f:
            if sys.version_info > (3, 0):
                objects.append(pkl.load(f, encoding='latin1'))
            else:
                objects.append(pkl.load(f))
    ...
    train_idx_orig = parse_index_file(
        "/mnt/d/jjpeng/ljzhao/text-gcn/data/{}.train.index".format(dataset_str))
...
def sparse_to_array(sparse_mx):
    ...
    def to_array(mx):
        ...
        for i in range(coords.shape[0]):
            coord = coords[i]
            new_array[coord[0]][coord[1]]=mx.data[i]
        return new_array

    if isinstance(sparse_mx, list):
        for i in range(len(sparse_mx)):
            sparse_mx[i] = to_array(sparse_mx[i])
    else:
        sparse_mx = to_array(sparse_mx)
...
def preprocess_features(features):
    ...
    return sparse_to_array(features)
...
def preprocess_adj(adj):
    ...
    return sparse_to_array(adj_normalized)
